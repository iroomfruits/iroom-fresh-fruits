(function(){
  ...
  const premiumVisuals={
    spring:['premium_variants/spring_01_금실딸기.png','premium_variants/spring_02_성주참외.png','premium_variants/spring_03_대저토마토.png','premium_variants/spring_04_샤인머스캣.png'],
    summer:['premium_variants/summer_01_백도복숭아.png','premium_variants/summer_02_고당도수박.png','premium_variants/summer_03_샤인머스캣.png','premium_variants/summer_04_머스크멜론.png'],
  ...
  function applySeason(key){
    ...
    $$('.premium-fruit-card:not(.seasonal-fruit-card)').forEach((card,i)=>{const n=(data.premiumFruits||[])[i],m=fruitMeta[n];if(!n||!m)return;card.dataset.fruit=n;const im=$('img',card),b=$('b',card),sm=$('small',card);if(im){im.src=assets+((premiumVisuals[current]||[])[i]||m[0]);im.alt=n+' 프리미엄 이미지'}if(b)b.textContent=n;if(sm)sm.textContent=m[1]});
