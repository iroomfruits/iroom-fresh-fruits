...
const INSTALL_DISMISS_KEY='iroom_pwa_install_collapsed_at_v60_6';
...
function refreshInstallUI(){
 const card=installCard(),fab=installFab(),standalone=isStandalone();
 if(card)card.classList.toggle('is-installed',standalone);
 if(standalone){
   if(card)card.hidden=false;
   if(fab)fab.hidden=true;
   setInstallButtonLabel('이룸 앱 홈으로 가기');
   setPrimaryDisabled(false);
   setInstallStatus('설치 완료 · 현재 이룸 앱으로 실행 중입니다. 버튼을 누르면 앱 홈으로 이동합니다.');
   return;
 }
 setPrimaryDisabled(false);
 ...
 if(deferredPrompt){setInstallButtonLabel('이룸 앱 설치하기');setInstallStatus('설치 준비 완료 · Chrome / Edge / Android');return}
 if(isIOS()){setInstallButtonLabel('iPhone 설치 방법');setInstallStatus('Safari 공유 버튼 → 홈 화면에 추가');return}
 setInstallButtonLabel('이룸 앱 설치하기');
 setInstallStatus(isAndroid()?'Chrome 메뉴 또는 설치 버튼으로 홈 화면에 추가할 수 있어요.':'Chrome / Edge에서 설치할 수 있어요. 설치창이 뜨지 않으면 버튼을 눌러 설치 방법을 확인하세요.');
}
function installGuideHtml(){
 if(isIOS())return `<ol><li><b>Safari</b>에서 현재 이룸 홈페이지를 엽니다.</li><li>화면 아래 또는 위의 <b>공유</b> 버튼을 누릅니다.</li><li><b>홈 화면에 추가</b>를 선택합니다.</li><li>오른쪽 위 <b>추가</b>를 누르면 이룸 로고 아이콘이 홈 화면에 생성됩니다.</li></ol><p>설치 후에는 홈 화면의 이룸 아이콘을 눌러 일반 앱처럼 사용할 수 있습니다.</p>`;
 ...
 if(isAndroid())return `<ol><li>Chrome 오른쪽 위 <b>⋮</b> 메뉴를 엽니다.</li><li><b>앱 설치</b> 또는 <b>홈 화면에 추가</b>를 선택합니다.</li><li><b>설치</b>를 누르면 이룸 로고 아이콘이 홈 화면에 생성됩니다.</li></ol>`;
 return `<ol><li>Chrome 또는 Edge 주소창의 <b>설치 아이콘</b>을 누르거나 오른쪽 위 메뉴를 엽니다.</li><li><b>앱 설치</b> 또는 <b>이 사이트를 앱으로 설치</b>를 선택합니다.</li><li>설치가 끝나면 바탕화면/시작 메뉴에 이룸 로고 아이콘이 생성됩니다.</li></ol><p>브라우저가 설치창을 제공하지 않는 경우에도 이 안내대로 설치할 수 있습니다.</p>`;
}
...
async function requestAppInstall(){
 if(isStandalone()){
   refreshInstallUI();
   if(location.pathname!=='/'&&location.pathname!=='/index.html'){location.assign('/');return}
 ...
 if(deferredPrompt){
   ...
   try{await promptEvent.prompt();const choice=await promptEvent.userChoice;if(choice?.outcome==='accepted'){try{localStorage.removeItem(INSTALL_DISMISS_KEY)}catch(_){}showToast('이룸 앱 설치를 시작합니다.')}refreshInstallUI()}catch(_){refreshInstallUI();openInstallGuide()}
   return;
 }
 openInstallGuide();
}
...
window.addEventListener('appinstalled',()=>{deferredPrompt=null;try{localStorage.removeItem(INSTALL_DISMISS_KEY)}catch(_){}closeInstallGuide();refreshInstallUI();showToast('이룸 앱이 설치되었습니다.')});
...
(async()=>{if(previewMode)document.body.classList.add('admin-preview-mode');await Promise.all([loadProducts(),loadHomepageConfig()]);applySeason(current);updateCartCount();updateAccount();try{if(!previewMode&&'serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=60.5').catch(()=>{})}catch(_){}})();
})();
