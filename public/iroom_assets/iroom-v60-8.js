...
const INSTALL_DISMISS_KEY='iroom_pwa_install_collapsed_at_v60_8';
const INSTALL_CONFIRMED_KEY='iroom_pwa_installed_confirmed_v60_8';
...
function refreshInstallUI(){
 ...
 if(installed){
   ...
   setInstallStatus('설치 완료 · 지금 이룸 앱으로 실행 중입니다.');
   return;
 }
 setPrimaryDisabled(false);setPrimaryBusy(false);
 ...
 if(isInAppBrowser()){
   setInstallButtonLabel(isIOS()?'Safari에서 열어 설치':'Chrome에서 열어 설치');
 ...
 if(deferredPrompt||window.__iroomInstallPrompt){
   ...
   setInstallButtonLabel('한 번 눌러 이룸 앱 설치');
   setInstallStatus('설치 준비 완료 · 버튼을 누르면 시스템 설치창이 바로 열립니다.');
   return;
 }
 if(isIOS()){
   setInstallButtonLabel('iPhone 홈 화면에 추가');
...
function installGuideHtml(){
 if(isInAppBrowser()&&isIOS())return `<div class="install-quick"><strong>iPhone은 Safari에서 설치해요</strong><p>아래 주소 복사 → Safari에서 열기 → 공유(□↑) → <b>홈 화면에 추가</b></p></div><button class="app-install-guide-action" type="button" data-copy-install-url>홈페이지 주소 복사</button>`;
 if(isInAppBrowser()&&isAndroid())return `<div class="install-quick"><strong>Chrome으로 열면 바로 설치할 수 있어요</strong><p>아래 버튼을 누른 뒤 이룸 홈페이지에서 <b>앱 설치</b>를 한 번 더 누르세요.</p></div><button class="app-install-guide-action" type="button" data-open-chrome>Chrome에서 열기</button>`;
 if(isIOS())return `<div class="install-quick ios"><strong>iPhone 설치는 딱 2단계</strong><div class="install-quick-step"><b>1</b><span>Safari의 <em>공유 □↑</em> 버튼 누르기</span></div><div class="install-quick-step"><b>2</b><span><em>홈 화면에 추가</em> → <em>추가</em></span></div><p>설치되면 홈 화면에 이룸 로고 아이콘이 생깁니다.</p></div>`;
 if(isEdge())return `<div class="install-quick"><strong>Edge가 자동 설치창을 제공하지 않았어요</strong><p>주소창 오른쪽의 <b>앱 설치 아이콘</b>을 한 번만 누르면 됩니다. 설치 후 바탕화면/시작 메뉴에는 이룸 로고가 표시됩니다.</p></div>`;
 if(isAndroid())return `<div class="install-quick"><strong>설치창이 아직 준비되지 않았어요</strong><p>잠시 후 이 창을 닫고 <b>이룸 앱 설치</b>를 다시 한 번 눌러주세요. 그래도 안 뜨면 Chrome 메뉴의 <b>앱 설치</b>를 선택하세요.</p></div>`;
 return `<div class="install-quick"><strong>브라우저 설치 기능을 확인해 주세요</strong><p>Chrome/Edge 주소창의 설치 아이콘을 누르면 이룸 앱으로 설치됩니다.</p></div>`;
}
...
async function presentInstallPrompt(promptEvent){
 ...
 try{
   ...
   }else setInstallStatus('설치를 취소했습니다. 원할 때 다시 설치할 수 있어요.');
   refreshInstallUI();
...
async function requestAppInstall(){
 ...
 if(!promptEvent&&isChromiumFamily()){
   setPrimaryBusy(true);
   setInstallButtonLabel('설치창 준비 중…');
   setInstallStatus('몇 초만 기다려 주세요. 설치창이 준비되면 자동으로 열립니다.');
