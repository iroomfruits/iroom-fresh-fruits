...
const INSTALL_DISMISS_KEY='iroom_pwa_install_collapsed_at_v60_7';
const INSTALL_CONFIRMED_KEY='iroom_pwa_installed_confirmed_v60_7';
const installButtons=()=>$$('[data-install]');
const installPrimary=()=> $('.app-install-primary[data-install]');
...
// 중요: 단순히 display-mode만 보고 '설치됨'으로 판정하지 않는다.
// 실제 PWA start_url(?source=pwa)로 실행했거나 appinstalled 이벤트가 확인된 경우만 설치 완료로 본다.
...
function setInstallButtonLabel(text){installButtons().forEach(btn=>btn.textContent=text)}
...
function refreshInstallUI(){
 ...
 if(installed){
   ...
   setInstallButtonLabel('이룸 앱 홈으로 가기');
   setPrimaryDisabled(false);
   setInstallStatus('설치 완료 · 현재 이룸 앱으로 실행 중입니다.');
   return;
 }
 setPrimaryDisabled(false);
 const collapsed=installCollapsedRecently();
 if(card)card.hidden=collapsed;
 if(fab)fab.hidden=!collapsed;
 if(isInAppBrowser()){
   setInstallButtonLabel(isIOS()?'Safari에서 열어 설치하기':'Chrome에서 열어 설치하기');
   setInstallStatus('카카오·네이버·인스타 내부 브라우저에서는 설치가 제한될 수 있어요.');
   return;
 }
 if(deferredPrompt){setInstallButtonLabel('이룸 앱 설치하기');setInstallStatus('설치 준비 완료 · 버튼을 누르면 설치창이 열립니다.');return}
 if(isIOS()){setInstallButtonLabel('iPhone 앱 설치하기');setInstallStatus('Safari 공유 → 홈 화면에 추가 → Open as Web App');return}
 if(isAndroid()){setInstallButtonLabel('Android 앱 설치하기');setInstallStatus('Chrome에서 설치창을 준비 중입니다. 안 뜨면 설치 방법을 안내합니다.');return}
 setInstallButtonLabel('이룸 앱 설치하기');
 setInstallStatus('Chrome / Edge에서 앱처럼 설치할 수 있습니다.');
}
...
function installGuideHtml(){
 if(isInAppBrowser()&&isIOS())return `<p><b>현재 앱 안의 브라우저에서는 iPhone 홈 화면 설치가 되지 않습니다.</b></p><ol><li>오른쪽 위 또는 아래의 <b>… / 공유</b> 메뉴를 누릅니다.</li><li><b>Safari에서 열기</b>를 선택합니다.</li><li>Safari에서 <b>공유 → 홈 화면에 추가</b>를 누릅니다.</li><li><b>Open as Web App</b>을 켠 뒤 <b>추가</b>를 누릅니다.</li></ol><button class="app-install-guide-action" type="button" data-copy-install-url>홈페이지 주소 복사</button>`;
 if(isInAppBrowser()&&isAndroid())return `<p><b>현재 앱 안의 브라우저에서는 설치창이 제한될 수 있습니다.</b></p><ol><li>아래 버튼으로 <b>Chrome</b>에서 이룸 홈페이지를 엽니다.</li><li>홈페이지의 <b>이룸 앱 설치하기</b> 버튼을 다시 누릅니다.</li><li>설치가 끝나면 이룸 로고 아이콘이 홈 화면에 생성됩니다.</li></ol><button class="app-install-guide-action" type="button" data-open-chrome>Chrome에서 열기</button>`;
 if(isIOS())return `<ol><li><b>Safari</b>에서 현재 이룸 홈페이지를 엽니다.</li><li>화면 아래 또는 위의 <b>공유</b> 버튼을 누릅니다.</li><li><b>홈 화면에 추가</b>를 선택합니다.</li><li><b>Open as Web App</b>을 켭니다.</li><li>오른쪽 위 <b>추가</b>를 누르면 이룸 로고 아이콘이 홈 화면에 생성됩니다.</li></ol><p>Apple의 iPhone 웹 앱 설치 방식은 Safari의 홈 화면 추가 절차를 사용합니다.</p>`;
 if(isEdge())return `<ol><li>Edge 오른쪽 위 <b>…</b> 메뉴를 엽니다.</li><li><b>앱</b> → <b>이 사이트를 앱으로 설치</b>를 선택합니다.</li><li><b>설치</b>를 누르면 바탕화면/시작 메뉴에 이룸 로고 아이콘이 생성됩니다.</li></ol>`;
 if(isAndroid())return `<ol><li><b>Chrome</b>에서 이룸 홈페이지를 엽니다.</li><li>오른쪽 위 <b>⋮</b> 메뉴에서 <b>앱 설치</b> 또는 <b>홈 화면에 추가</b>를 선택합니다.</li><li><b>설치</b>를 누르면 이룸 로고 아이콘이 홈 화면에 생성됩니다.</li></ol><p>설치 가능 조건이 충족되면 홈페이지의 설치 버튼을 누르는 즉시 Chrome 설치창이 열립니다.</p>`;
 return `<ol><li>Chrome 또는 Edge 주소창의 <b>설치 아이콘</b>을 누르거나 오른쪽 위 메뉴를 엽니다.</li><li><b>앱 설치</b> 또는 <b>이 사이트를 앱으로 설치</b>를 선택합니다.</li><li>설치가 끝나면 바탕화면/시작 메뉴에 이룸 로고 아이콘이 생성됩니다.</li></ol>`;
}
...
async function requestAppInstall(){
 ...
 if(deferredPrompt){
   ...
   try{
     ...
     if(choice?.outcome==='accepted'){try{localStorage.removeItem(INSTALL_DISMISS_KEY)}catch(_){}showToast('이룸 앱 설치를 시작합니다.')}
     refreshInstallUI();
   }catch(_){refreshInstallUI();openInstallGuide()}
   return;
 }
 openInstallGuide();
}
window.addEventListener('beforeinstallprompt',e=>{e.preventDefault();deferredPrompt=e;try{localStorage.removeItem(INSTALL_DISMISS_KEY)}catch(_){}refreshInstallUI()});
window.addEventListener('appinstalled',()=>{deferredPrompt=null;markInstalled();try{localStorage.removeItem(INSTALL_DISMISS_KEY)}catch(_){}closeInstallGuide();refreshInstallUI();showToast('이룸 앱이 설치되었습니다.')});
...
(async()=>{if(previewMode)document.body.classList.add('admin-preview-mode');await Promise.all([loadProducts(),loadHomepageConfig()]);applySeason(current);updateCartCount();updateAccount();try{if(!previewMode&&'serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=60.7').catch(()=>{})}catch(_){}})();
})();
