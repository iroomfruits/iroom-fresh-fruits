...
const INSTALL_DISMISS_KEY='iroom_pwa_install_dismissed_at_v60_4';
...
function installDismissedRecently(){try{const t=Number(localStorage.getItem(INSTALL_DISMISS_KEY)||0);return t&&Date.now()-t<7*24*60*60*1000}catch(_){return false}}
...
function refreshInstallUI(){
 ...
 if(isStandalone()){
   ...
   setInstallStatus('현재 이 기기에 이룸 앱이 설치되어 있습니다.');
   return;
 }
 if(card)card.hidden=installDismissedRecently();
 if(deferredPrompt){setInstallButtonLabel('이룸 앱 설치하기');setInstallStatus('설치 준비 완료 · Chrome / Edge / Android 지원');return}
 if(isIOS()){setInstallButtonLabel('iPhone 설치 방법');setInstallStatus('Safari 공유 버튼 → 홈 화면에 추가');return}
 setInstallButtonLabel('이룸 앱 설치하기');
 setInstallStatus(isAndroid()?'Chrome 메뉴 또는 설치 버튼으로 홈 화면에 추가할 수 있어요.':'Chrome / Edge에서 설치할 수 있어요.');
}
function installGuideHtml(){
 if(isIOS())return `<ol><li><b>Safari</b>에서 현재 이룸 홈페이지를 엽니다.</li><li>화면 아래 또는 위의 <b>공유</b> 버튼을 누릅니다.</li><li><b>홈 화면에 추가</b>를 선택합니다.</li><li>오른쪽 위 <b>추가</b>를 누르면 이룸 아이콘이 홈 화면에 생성됩니다.</li></ol><p>설치 후 실행하면 일반 앱처럼 별도 창에서 이룸 홈페이지를 사용할 수 있습니다.</p>`;
 if(isEdge())return `<ol><li>Edge 오른쪽 위 <b>…</b> 메뉴를 엽니다.</li><li><b>앱</b> → <b>이 사이트를 앱으로 설치</b>를 선택합니다.</li><li><b>설치</b>를 누르면 바탕화면/시작 메뉴에 이룸 아이콘이 생성됩니다.</li></ol>`;
 if(isAndroid())return `<ol><li>Chrome 오른쪽 위 <b>⋮</b> 메뉴를 엽니다.</li><li><b>앱 설치</b> 또는 <b>홈 화면에 추가</b>를 선택합니다.</li><li><b>설치</b>를 누르면 이룸 아이콘이 홈 화면에 생성됩니다.</li></ol>`;
 return `<ol><li>Chrome 주소창의 <b>설치 아이콘</b>을 누르거나 오른쪽 위 <b>⋮</b> 메뉴를 엽니다.</li><li><b>저장 및 공유</b> 또는 <b>앱 설치</b> 메뉴에서 이룸을 설치합니다.</li><li>설치가 끝나면 바탕화면/시작 메뉴에 이룸 아이콘이 생성됩니다.</li></ol><p>설치 메뉴가 보이지 않으면 페이지를 새로고침한 뒤 다시 시도해 주세요.</p>`;
}
...
async function requestAppInstall(){
 ...
 if(deferredPrompt){
   ...
   try{await promptEvent.prompt();const choice=await promptEvent.userChoice;if(choice?.outcome==='accepted'){const card=installCard();if(card)card.hidden=true;showToast('이룸 앱 설치를 시작합니다.')}else refreshInstallUI()}catch(_){openInstallGuide()}
   return;
 }
 openInstallGuide();
}
...
document.addEventListener('click',e=>{
 ...
 if(e.target.closest('[data-install-dismiss]')){e.preventDefault();try{localStorage.setItem(INSTALL_DISMISS_KEY,String(Date.now()))}catch(_){}const card=installCard();if(card)card.hidden=true;return}
});
refreshInstallUI();
...
(async()=>{if(previewMode)document.body.classList.add('admin-preview-mode');await Promise.all([loadProducts(),loadHomepageConfig()]);applySeason(current);updateCartCount();updateAccount();try{if(!previewMode&&'serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=60.4').catch(()=>{})}catch(_){}})();
})();
