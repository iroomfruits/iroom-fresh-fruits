...
const INSTALL_DISMISS_KEY='iroom_pwa_install_collapsed_at_v60_5';
...
function refreshInstallUI(){
 ...
 if(standalone){
   ...
   setInstallButtonLabel('이룸 앱 설치됨');
   setPrimaryDisabled(true);
   setInstallStatus('현재 이 창은 설치된 이룸 앱입니다. 홈 화면/바탕화면의 이룸 아이콘으로 실행 중이에요.');
   return;
 }
...
async function requestAppInstall(){
 if(isStandalone()){refreshInstallUI();showToast('이미 이룸 앱으로 실행 중입니다.');return}
 if(deferredPrompt){
