...
document.addEventListener('click',e=>{
 ...
 const add=e.target.closest('[data-cart-add]');if(add){e.preventDefault();e.stopPropagation();addToCart(add.dataset.name,qtyFrom(add),add.dataset.visual||'');openModal('cart');return}
 ...
 const checkout=e.target.closest('[data-checkout]');if(checkout){e.preventDefault();renderCheckout();return}
...
(async()=>{await loadProducts();applySeason(current);updateCartCount();updateAccount();try{if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=45').catch(()=>{})}catch(_){}})();
})();
