...
window.addEventListener('message',e=>{if(e.origin!==location.origin)return;const d=e.data||{};if(d.type==='iroom-preview-open-detail'){showFruit(d.baseName||'',d.visual||'',d.displayName||'',d.editKey||'')}});
...
(async()=>{if(previewMode)document.body.classList.add('admin-preview-mode');await Promise.all([loadProducts(),loadHomepageConfig()]);applySeason(current);updateCartCount();updateAccount();try{if(!previewMode&&'serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=57').catch(()=>{})}catch(_){}})();
})();
