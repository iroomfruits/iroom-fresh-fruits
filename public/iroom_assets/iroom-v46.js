...
document.addEventListener('click',e=>{
 ...
 const buy=e.target.closest('[data-buy-now]');if(buy){e.preventDefault();const item=makeCartItem(buy.dataset.name,qtyFrom(buy),buy.dataset.visual||'');renderCheckout([item],'buy-now');return}
...
(async()=>{await loadProducts();applySeason(current);updateCartCount();updateAccount();try{if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=46').catch(()=>{})}catch(_){}})();
})();
