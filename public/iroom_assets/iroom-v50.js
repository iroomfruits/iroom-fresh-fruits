...
function productCard(name,img,premium=false,index=0){const m=meta(name),p=productFor(name),v=img||m[0];const base=alias[name]||name;const watercolor=premium&&/premium_watercolor_v49/.test(v);const badge=premium&&!watercolor?`<span class="badge ${imported.has(base)?'imported-badge':''}">${imported.has(base)?'수입 프리미엄':'국산 프리미엄'}</span>`:'';const signature=premium&&!watercolor?`<span class="premium-card-no">IROOM ${String(index+1).padStart(2,'0')}</span>`:'';return `<article class="product-card commerce-card clickable-product ${premium?'premium-signature-card':''} ${watercolor?'premium-watercolor-card':''}" data-card-fruit="${esc(name)}" data-card-visual="${esc(v)}">${signature}${badge}<button type="button" class="image-wrap product-image-button ${premium?'premium-image-button':''}" data-fruit="${esc(name)}" data-visual="${esc(v)}" aria-label="${esc(name)} 상세보기"><img src="${A+v}" alt="${esc(name)}"></button><h3>${esc(name)}</h3><p>${m[1]}</p><div class="mini-sale"><span>${esc(p.unit||'구성 상담')}</span><b>${displayPrice(p)}</b></div>${commerceButtons(name,v,true)}</article>`}
...
(async()=>{await loadProducts();applySeason(current);updateCartCount();updateAccount();try{if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=50').catch(()=>{})}catch(_){}})();
})();
