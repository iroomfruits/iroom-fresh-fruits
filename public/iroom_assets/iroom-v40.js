...
function stopCarousel(sel){const state=carouselTimers.get(sel);if(state?.raf)cancelAnimationFrame(state.raf);if(state?.resumeTimer)clearTimeout(state.resumeTimer);carouselTimers.delete(sel)}
function setupSeamlessCarousel(sel,speed=.36){
 ...
 const bf=document.createDocumentFragment(),af=document.createDocumentFragment();before.forEach(n=>bf.appendChild(n));after.forEach(n=>af.appendChild(n));el.prepend(bf);el.append(af);
 let paused=false,resumeTimer=0,raf=0,setWidth=0,last=performance.now();
 const measure=()=>{const firstOriginal=originals[0],firstBefore=before[0];setWidth=Math.max(1,firstOriginal.offsetLeft-firstBefore.offsetLeft);if(Math.abs(el.scrollLeft-setWidth)>4)el.scrollLeft=setWidth};
 requestAnimationFrame(measure);
 const normalize=()=>{if(!setWidth)return;if(el.scrollLeft>=setWidth*2)el.scrollLeft-=setWidth;else if(el.scrollLeft<=1)el.scrollLeft+=setWidth};
 const frame=now=>{const dt=Math.min(40,now-last);last=now;if(!paused&&!document.hidden&&!overlay?.classList.contains('open')){el.scrollLeft+=speed*(dt/16.67);normalize()}raf=requestAnimationFrame(frame)};
 const pause=()=>{paused=true;clearTimeout(resumeTimer)};
 const resumeSoon=()=>{clearTimeout(resumeTimer);resumeTimer=setTimeout(()=>{normalize();paused=false},1200)};
 el.addEventListener('mouseenter',pause);el.addEventListener('mouseleave',resumeSoon);el.addEventListener('focusin',pause);el.addEventListener('focusout',resumeSoon);el.addEventListener('pointerdown',pause);el.addEventListener('pointerup',resumeSoon);el.addEventListener('touchstart',pause,{passive:true});el.addEventListener('touchend',resumeSoon,{passive:true});
 window.addEventListener('resize',()=>requestAnimationFrame(measure),{passive:true});
 raf=requestAnimationFrame(frame);carouselTimers.set(sel,{get raf(){return raf},get resumeTimer(){return resumeTimer}})
}
function setupAllCarousels(){setupSeamlessCarousel('#todayGrid',.34);setupSeamlessCarousel('#premiumGrid',.30);setupSeamlessCarousel('#seasonalGrid',.32);setupSeamlessCarousel('#libraryGrid',.38)}

(async()=>{await loadProducts();applySeason(current);updateCartCount();updateAccount();try{if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=40').catch(()=>{})}catch(_){}})();
})();
