...
const seasons={
 ...
 autumn:{label:'가을',heroCopy:'깊어가는 계절이 전하는 가장 특별한 맛을 오늘의 상태로 골라드립니다.',hero:['이지플','나주배','샤인머스캣','대봉','석류'],today:[['이지플','국내 산지','당일 선별'],['나주배','전남 나주','입고 상태 확인'],['샤인머스캣','국내 산지','당일 상태 확인'],['대봉','경남·전남 산지','후숙 상태 확인'],['석류','입고별 안내','당일 선별'],['망고','태국·페루 등','숙도 확인'],['자몽','미국·이스라엘 등','입고 상태 확인'],['아보카도','멕시코·페루 등','숙도 확인'],['망고스틴','태국 등','입고 상태 확인'],['패션후르츠','베트남 등','당일 선별']],premium:['이지플','나주배','프리미엄 샤인머스캣','하우스 자두'],premiumImg:['premium_box_v53/autumn_01.webp','premium_box_v53/autumn_02.webp','premium_box_v53/autumn_03.webp','premium_box_v53/autumn_04.webp'],seasonal:['포도','밤','블랙베리','키위','파인애플','용과','망고스틴','패션후르츠'],library:['석류','포도','블랙베리','자몽','샤인머스캣','사과','배','키위','블루베리','제주감귤','망고','아보카도'],curation:'feature_art/autumn_curation.jpg',gift:'feature_art/autumn_gift.jpg'},
...
function sectionEntries(section,season=current){
 ...
 const source=Array.isArray(custom)&&custom.length?custom:defaultSectionEntries(section,season);
...
function heroCard(name,i,visual=''){const v=slotView('hero',i,name,visual||meta(name)[0]);return `<div class="hero-fruit f${i+1}"${slotData(v)}${previewAttrs(v)}><img src="${absoluteVisual(v.visual)}" alt="${esc(v.displayName)}" decoding="async" loading="eager" fetchpriority="high"><span>${esc(v.displayName)}</span></div>`}
...
function applySeason(key){
 ...
 const heroTitle=String(sc.heroTitle||`${d.label} 프리미엄\n과일`);if($('[data-hero-title]'))$('[data-hero-title]').innerHTML=heroTitle.split(/\n/).map(esc).join('<br>');
 if($('[data-hero-copy]'))$('[data-hero-copy]').textContent=sc.heroCopy||d.heroCopy;
...
function renderSearch(){
 setModal('SEARCH','과일 찾기','과일명뿐 아니라 산지·구성·맛·국산/수입까지 검색할 수 있습니다.',`<div class="search-row"><input id="fruitSearch" placeholder="예: 나주, 선물, 2kg, 국산, 달콤한 과일" autocomplete="off"><button id="fruitSearchBtn">검색</button></div><div class="search-hint">과일 이름을 몰라도 산지나 용도로 찾아보세요.</div><div class="search-results rich-search-results" id="searchResults"></div>`);
 const names=[...new Set([...Object.keys(fruits),...storeProducts.map(x=>x.name).filter(Boolean)])];
 const seasonTags=name=>Object.entries(seasons).filter(([,d])=>[...(d.hero||[]),...(d.premium||[]),...(d.seasonal||[]),...(d.library||[]),...(d.today||[]).map(x=>x[0])].some(n=>norm(n)===norm(name)||norm(alias[n])===norm(name))).map(([k])=>({spring:'봄',summer:'여름',autumn:'가을',winter:'겨울'}[k])).join(' ');
 const index=names.map(name=>{const p=productFor(name),base=alias[name]||name,tags=[name,base,p.origin,p.unit,p.description,meta(name)[1],imported.has(base)?'수입':'국산',seasonTags(name),'선물 과일 제철 프리미엄'].join(' ').toLowerCase();return {name,p,tags}});
 const draw=q=>{const raw=String(q||'').trim().toLowerCase(),tokens=raw.split(/\s+/).filter(Boolean);let rows=index.filter(x=>tokens.every(t=>x.tags.includes(t)));rows.sort((a,b)=>{const an=a.name.toLowerCase(),bn=b.name.toLowerCase();const ar=raw&&an.startsWith(raw)?0:raw&&an.includes(raw)?1:2,br=raw&&bn.startsWith(raw)?0:raw&&bn.includes(raw)?1:2;return ar-br||a.name.localeCompare(b.name,'ko')});$('#searchResults').innerHTML=rows.slice(0,36).map(x=>`<button class="rich-search-item" data-fruit="${esc(x.name)}"><b>${esc(x.name)}</b><small>${esc(x.p.origin||'입고별 안내')} · ${esc(x.p.unit||'구성 상담')}</small><strong>${displayPrice(x.p)}</strong></button>`).join('')||'<p class="search-empty">조건에 맞는 과일이 없어요. 다른 이름이나 산지로 검색해보세요.</p>'};
 draw('');let timer=0;$('#fruitSearchBtn').onclick=()=>draw($('#fruitSearch').value);$('#fruitSearch').oninput=e=>{clearTimeout(timer);timer=setTimeout(()=>draw(e.target.value),120)};$('#fruitSearch').onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();draw(e.target.value)}}
}
...
const LAST_ORDER_KEY='iroom_last_order_no_v60';
function formatOrderDate(v){try{return new Intl.DateTimeFormat('ko-KR',{year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'}).format(new Date(v))}catch(_){return String(v||'')}}
function orderStatusTone(status=''){const x=String(status);if(/완료|배송완료/.test(x))return 'done';if(/취소|환불/.test(x))return 'warn';return 'active'}
function renderOrderLookup(prefill=''){
 let last=prefill;try{last=last||localStorage.getItem(LAST_ORDER_KEY)||''}catch(_){}
 setModal('ORDER CARE','비회원 주문조회','주문번호와 주문할 때 입력한 연락처만 있으면 바로 확인할 수 있습니다.',`<div class="order-lookup-intro"><span>◎</span><div><b>회원가입 없이 주문 상태 확인</b><small>주소·이메일 등 개인정보는 조회 화면에 표시하지 않습니다.</small></div></div><form id="orderLookupForm" class="order-lookup-form"><label><span>주문번호</span><input name="order_no" autocomplete="off" value="${esc(last)}" placeholder="예: IROOM-..." required></label><label><span>주문자 연락처</span><input name="phone" autocomplete="tel" inputmode="tel" placeholder="010-0000-0000" required></label><button class="primary" type="submit">주문 조회하기</button><p class="order-lookup-error" id="orderLookupError" hidden></p></form><div id="orderLookupResult"></div>`);
 showOverlay('order-lookup');
 const form=$('#orderLookupForm'),error=$('#orderLookupError'),result=$('#orderLookupResult');
 form.onsubmit=async e=>{e.preventDefault();error.hidden=true;result.innerHTML='';const btn=form.querySelector('button[type="submit"]'),original=btn.textContent;btn.disabled=true;btn.textContent='조회 중…';const fd=new FormData(form);try{const r=await fetch('/api/orders/lookup',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({order_no:fd.get('order_no'),phone:fd.get('phone')})});const d=await r.json();if(!r.ok)throw new Error(d.error||'주문조회 중 오류가 발생했습니다.');const o=d.order||{};try{localStorage.setItem(LAST_ORDER_KEY,o.order_no||'')}catch(_){}const region=({normal:'일반지역',jeju:'제주도',remote:'제주 외 도서산간'}[o.shipping_region]||'일반지역');result.innerHTML=`<section class="order-lookup-result"><div class="order-lookup-result-head"><div><small>ORDER ${esc(o.order_no||'')}</small><h3>주문 상태</h3></div><span class="order-status ${orderStatusTone(o.status)}">${esc(o.status||'주문접수')}</span></div><div class="order-status-grid"><div><small>결제 상태</small><b>${esc(o.payment_status||'입금대기')}</b></div><div><small>배송지역</small><b>${esc(region)}</b></div><div><small>주문일시</small><b>${esc(formatOrderDate(o.created_at))}</b></div><div><small>결제방법</small><b>${esc(o.payment_method||'무통장입금')}</b></div></div><div class="order-lookup-items">${(o.items||[]).map(x=>`<div><span>${esc(x.product_name||'상품')} × ${Number(x.qty||0)}</span><b>${won(Number(x.line_total||0))}</b></div>`).join('')}</div><div class="order-lookup-total"><span>상품금액 <b>${won(Number(o.subtotal||0))}</b></span><span>배송비 <b>${Number(o.shipping_fee||0)?won(Number(o.shipping_fee||0)):'무료'}</b></span><strong>총 결제금액 <b>${won(Number(o.total_amount||0))}</b></strong></div><div class="order-care-actions"><button type="button" class="secondary" data-copy-order data-order-no="${esc(o.order_no||'')}">주문번호 복사</button><button type="button" class="primary" data-kakao-chat>주문 상담</button></div></section>`}catch(err){error.textContent=err.message||'주문조회 중 오류가 발생했습니다.';error.hidden=false}finally{btn.disabled=false;btn.textContent=original}};
}
let kakaoPostcodeLoading=null;
function ensureKakaoPostcode(){if(window.kakao?.Postcode||window.daum?.Postcode)return Promise.resolve(true);if(kakaoPostcodeLoading)return kakaoPostcodeLoading;kakaoPostcodeLoading=new Promise((resolve,reject)=>{const sc=document.createElement('script');sc.src='https://t1.kakaocdn.net/mapjsapi/bundle/postcode/prod/postcode.v2.js';sc.async=true;sc.onload=()=>window.kakao?.Postcode||window.daum?.Postcode?resolve(true):reject(new Error('주소검색을 불러오지 못했습니다.'));sc.onerror=()=>reject(new Error('주소검색을 불러오지 못했습니다.'));document.head.appendChild(sc)}).catch(err=>{kakaoPostcodeLoading=null;throw err});return kakaoPostcodeLoading}
async function openPostcodeSearch(){const btn=$('[data-postcode-search]');if(btn){btn.disabled=true;btn.textContent='주소검색 불러오는 중…'}try{await ensureKakaoPostcode();const Postcode=window.kakao?.Postcode||window.daum?.Postcode;new Postcode({oncomplete:data=>{const post=$('#checkoutForm [name="postcode"]'),addr=$('#checkoutForm [name="address1"]'),detail=$('#checkoutForm [name="address2"]');if(post)post.value=data.zonecode||'';if(addr)addr.value=data.roadAddress||data.jibunAddress||'';if(detail){detail.focus();detail.scrollIntoView({behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth',block:'center'})}}}).open()}catch(_){showToast('주소검색을 열지 못했습니다. 주소를 직접 입력해 주세요.')}finally{if(btn){btn.disabled=false;btn.textContent='주소 검색'}}}
...
function renderGuestOrder(){
  if(!cart.length){
    overlay.dataset.kind='guest-checkout';
    setModal('GUEST ORDER','비회원 주문','회원가입 없이 장바구니 상품을 바로 주문할 수 있습니다.',`<div class="guest-order-steps" aria-label="비회원 주문 순서"><span class="active">1 · 상품 담기</span><span>2 · 주문자·배송 정보</span><span>3 · 주문 접수</span></div><div class="guest-order-empty"><span class="guest-order-empty-icon">◎</span><h3>비회원 장바구니가 비어 있어요.</h3><p>원하는 과일을 장바구니에 담은 뒤 다시 <b>비회원 주문</b>을 누르면, 장바구니와 주문자 정보 입력창이 한 화면에 바로 열립니다.</p><button type="button" class="primary" data-guest-shop>과일 담으러 가기</button></div>`);
    showOverlay('guest-checkout');
    return;
  }
  renderCheckout(cart,'guest');
}
function renderCheckout(items=cart,source='cart'){
  const isBuyNow=source==='buy-now';
  const isGuest=source==='guest';
  overlay.dataset.kind=isGuest?'guest-checkout':'checkout';
  const orderItems=(Array.isArray(items)?items:[]).map(x=>({...x,qty:Math.max(1,Math.min(20,Number(x.qty)||1))}));
  if(!orderItems.length){if(isGuest){renderGuestOrder();return}renderCart();showOverlay('cart');return}
  ...
  const checkoutSummary=`<div class="checkout-summary">${orderItems.map(x=>`<div><span>${esc(x.displayName||x.name)} × ${x.qty}</span><b>${won(Number(x.price||0)*Number(x.qty||0))}</b></div>`).join('')}<div><span>상품금액</span><b>${won(subtotal)}</b></div><div><span>배송비</span><b id="checkoutShippingFee">${shipping?won(shipping):'무료'}</b></div><strong><span>최종 결제예정금액</span><b id="checkoutGrandTotal">${won(total)}</b></strong>${freeFrom>0&&subtotal<freeFrom?`<small class="free-shipping-note">${won(freeFrom-subtotal)} 더 주문하면 일반지역 기본배송비가 무료입니다.</small>`:''}</div>`;
  const guestBag=`<div class="guest-order-steps" aria-label="비회원 주문 순서"><span class="active">1 · 장바구니 확인</span><span class="active">2 · 주문자·배송 정보</span><span>3 · 주문 접수</span></div><section class="guest-order-bag" aria-label="비회원 장바구니"><div class="guest-order-bag-head"><div><small>GUEST BAG</small><b>비회원 장바구니</b></div><button type="button" class="secondary" data-guest-open-cart>상품·수량 수정</button></div><div class="guest-order-bag-list">${orderItems.map(x=>`<div class="guest-order-bag-item"><div><b>${esc(x.displayName||x.name)} <em>× ${x.qty}</em></b><small>${esc(x.unit||'')}</small></div><strong>${won(Number(x.price||0)*Number(x.qty||0))}</strong></div>`).join('')}</div><div class="guest-order-bag-total"><span>상품금액 <b>${won(subtotal)}</b></span><span>배송비 <b id="checkoutShippingFee">${shipping?won(shipping):'무료'}</b></span><strong>최종 결제예정금액 <b id="checkoutGrandTotal">${won(total)}</b></strong>${freeFrom>0&&subtotal<freeFrom?`<small class="free-shipping-note">${won(freeFrom-subtotal)} 더 주문하면 일반지역 기본배송비가 무료입니다.</small>`:''}</div></section>`;
  setModal(isBuyNow?'BUY NOW':isGuest?'GUEST ORDER':'CHECKOUT',isBuyNow?'바로 구매':isGuest?'비회원 주문':'주문 정보 입력',isBuyNow?'선택한 상품만 바로 주문합니다. 기존 장바구니 상품은 그대로 유지됩니다.':isGuest?'비회원 장바구니와 주문자 정보를 한 화면에서 확인하고 바로 주문하세요.':'장바구니 상품의 주문 정보를 입력해 주세요.',`${isGuest?guestBag:checkoutSummary}<form id="checkoutForm" class="checkout-form"><div class="guest-checkout-note"><b>${isGuest?'비회원 주문자 정보':'비회원 주문 가능'}</b><span>${isGuest?'회원가입 없이 아래 필수 정보만 입력하면 주문이 접수됩니다.':'회원가입 없이 배송정보만 입력하면 주문할 수 있습니다.'}</span></div><div class="form-grid"><label><span class="field-label">주문자명 <em>필수</em></span><input name="customer_name" autocomplete="name" placeholder="주문자명" required></label><label><span class="field-label">연락처 <em>필수</em></span><input name="phone" autocomplete="tel" inputmode="tel" placeholder="010-0000-0000" required><small class="field-help">배송·주문 안내와 비회원 주문조회에 사용합니다.</small></label><label><span class="field-label">이메일 <em>선택</em></span><input name="email" type="email" autocomplete="email" inputmode="email" placeholder="주문 확인용 이메일"></label><div class="address-search-field"><label><span class="field-label">우편번호 <em>선택</em></span><input name="postcode" autocomplete="postal-code" inputmode="numeric" placeholder="우편번호"></label><button type="button" class="secondary postcode-search-btn" data-postcode-search>주소 검색</button></div><label class="full"><span class="field-label">배송지 주소 <em>필수</em></span><input name="address1" autocomplete="street-address" placeholder="도로명 주소" required></label><label class="full"><span class="field-label">상세주소 <em>선택</em></span><input name="address2" autocomplete="address-line2" placeholder="동·호수 등"></label><label class="full shipping-region-field"><span>배송지역</span><select name="shipping_region" id="shippingRegion"><option value="normal">일반지역</option><option value="jeju">제주도 (+${won(jejuExtra)})</option><option value="remote">제주 외 도서산간 (+${won(remoteExtra)})</option></select></label><label class="full"><span class="field-label">배송 메모 <em>선택</em></span><textarea name="memo" autocomplete="off" placeholder="부재 시 요청사항 등"></textarea></label></div><div class="checkout-notice">${isBuyNow?'바로 구매는 현재 선택한 상품만 주문됩니다.':isGuest?'비회원 주문은 계정 생성 없이 바로 접수됩니다.':'매일 시세와 입고 상태를 반영합니다.'} 기본 배송비 ${won(baseShipping)} · ${won(freeFrom)} 이상 일반지역 무료배송 · 제주 추가 ${won(jejuExtra)} · 제주 외 도서산간 추가 ${won(remoteExtra)}입니다. 지역 추가운임은 무료배송과 별도로 적용됩니다.${publicSite.courier?` 택배사: ${esc(publicSite.courier)}.`:' 택배사는 확정 후 안내합니다.'}</div><div class="payment-bank-box"><div><small>현재 결제방법 · 무통장입금</small><strong>${esc(paymentBank.name)} ${esc(paymentBank.account)}</strong><span>예금주 ${esc(paymentBank.holder)} · 입금자명은 주문자명과 동일하게 입력하면 확인이 빠릅니다.</span></div><button type="button" class="secondary" data-copy-bank>계좌 복사</button></div><div class="checkout-consents"><label><input type="checkbox" name="privacy_consent" value="yes" required><span><b>[필수] 개인정보 수집·이용 동의</b><small>주문·배송·고객응대에 필요한 이름, 연락처, 배송지 정보를 처리합니다.</small></span></label><details><summary>개인정보 안내 보기</summary><p>${esc(publicPolicies.privacyText||'주문 및 배송에 필요한 개인정보를 처리합니다.')}</p></details><label><input type="checkbox" name="terms_consent" value="yes" required><span><b>[필수] 구매조건 및 배송·교환·환불 안내 확인</b><small>상품·수량·금액·배송비와 신선식품 특성에 따른 교환·환불 안내를 확인했습니다.</small></span></label><details><summary>배송·교환·환불 안내 보기</summary><p>${esc(publicPolicies.shippingPolicyText||'신선식품 특성을 반영한 배송·교환·환불 정책을 적용합니다.')}</p></details></div><p class="checkout-form-error" id="checkoutFormError" hidden></p><div class="checkout-submit-bar"><div><small>최종 결제 예정금액</small><strong data-checkout-sticky-total>${won(total)}</strong></div><button class="primary checkout-submit">${isGuest?'비회원 주문 접수하기':'주문 접수하기'}</button></div></form>`);
  showOverlay(isGuest?'guest-checkout':'checkout');
  const form=$('#checkoutForm'),regionSelect=$('#shippingRegion'),shippingEl=$('#checkoutShippingFee'),totalEl=$('#checkoutGrandTotal');
  const updateShipping=()=>{const fee=calcShipping(regionSelect?.value||'normal');if(shippingEl)shippingEl.textContent=fee?won(fee):'무료';if(totalEl)totalEl.textContent=won(subtotal+fee);const sticky=$('[data-checkout-sticky-total]');if(sticky)sticky.textContent=won(subtotal+fee)};
  ...
  form.onsubmit=async e=>{
    ...
    const errorBox=$('#checkoutFormError');if(errorBox){errorBox.hidden=true;errorBox.textContent=''}
    if(!form.reportValidity())return;
    const fd=new FormData(form);const payload=Object.fromEntries(fd.entries());
    payload.privacy_consent=fd.get('privacy_consent')==='yes';payload.terms_consent=fd.get('terms_consent')==='yes';
    payload.items=orderItems.map(x=>({product_id:x.id,qty:x.qty}));payload.payment_method='무통장입금';
    const submitBtn=form.querySelector('.checkout-submit');const originalText=submitBtn?.textContent||'주문 접수하기';if(submitBtn){submitBtn.disabled=true;submitBtn.textContent='주문 접수 중…'}
    try{
      ...
      try{localStorage.setItem(LAST_ORDER_KEY,d.order_no||'')}catch(_){}
      const regionLabel=d.shipping_region_label||({'normal':'일반지역','jeju':'제주도','remote':'제주 외 도서산간'}[payload.shipping_region]||'일반지역');
      setModal('ORDER COMPLETE','주문이 접수되었습니다.',`주문번호 <b>${esc(d.order_no||'')}</b>`, `<div class="order-complete"><div class="order-complete-badge">ORDER RECEIVED</div><p>배송지역 <b>${esc(regionLabel)}</b><br>상품금액 <b>${won(d.subtotal??subtotal)}</b><br>배송비 <b>${Number(d.shipping_fee??shipping)===0?'무료':won(d.shipping_fee??shipping)}</b><br>총 결제 예정 금액 <b>${won(d.total_amount||0)}</b></p><div class="order-bank-confirm"><small>입금계좌</small><strong>${esc(d.bank?.name||'')} ${esc(d.bank?.account||'')}</strong><span>예금주 ${esc(d.bank?.holder||'')} · 주문자명과 같은 이름으로 입금하면 확인이 빠릅니다.</span></div><p class="order-complete-help">주문번호는 비회원 주문조회에 필요합니다. 캡처하거나 복사해 보관해 주세요.</p><div class="order-complete-actions"><button class="secondary" data-copy-order data-order-no="${esc(d.order_no||'')}">주문번호 복사</button><button class="secondary" data-copy-bank>계좌 복사</button><button class="primary" data-order-lookup data-order-no="${esc(d.order_no||'')}">주문 조회하기</button></div></div>`)
    }catch(err){if(errorBox){errorBox.textContent=err.message||'주문 처리 중 오류가 발생했습니다.';errorBox.hidden=false;errorBox.scrollIntoView({behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth',block:'center'})}else alert(err.message||'주문 처리 중 오류가 발생했습니다.')}
    finally{if(submitBtn&&document.body.contains(submitBtn)){submitBtn.disabled=false;submitBtn.textContent=originalText}}
  }
}
...
document.addEventListener('click',e=>{
 ...
 const co=e.target.closest('[data-copy-order]');if(co){e.preventDefault();const text=co.dataset.orderNo||'';navigator.clipboard?.writeText(text).then(()=>showToast('주문번호를 복사했습니다.')).catch(()=>prompt('주문번호를 복사해 주세요.',text));return}
 const ol=e.target.closest('[data-order-lookup]');if(ol){e.preventDefault();renderOrderLookup(ol.dataset.orderNo||'');return}
 const pc=e.target.closest('[data-postcode-search]');if(pc){e.preventDefault();openPostcodeSearch();return}
 const kc=e.target.closest('[data-kakao-chat]');if(kc){e.preventDefault();openKakaoChat();return}
 const guestOrder=e.target.closest('[data-guest-order]');if(guestOrder){e.preventDefault();e.stopPropagation();renderGuestOrder();return}
 const guestCart=e.target.closest('[data-guest-open-cart]');if(guestCart){e.preventDefault();renderCart();showOverlay('cart');return}
 const guestShop=e.target.closest('[data-guest-shop]');if(guestShop){e.preventDefault();closeModal();const target=document.getElementById('today')||document.getElementById('premium');if(target)setTimeout(()=>target.scrollIntoView({behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth',block:'start'}),40);return}
...
function renderPublicReviews(reviews=[]){const section=$('#reviewSection'),grid=$('#reviewGrid');if(!section||!grid)return;const rows=(Array.isArray(reviews)?reviews:[]).filter(x=>x&&x.text).slice(0,3);if(!rows.length){section.hidden=true;grid.innerHTML='';return}section.hidden=false;const fallback=['fruits/09_사과_apple.png','fruits/12_샤인머스캣_shine_muscat.png','fruits/10_배_pear.png'];grid.innerHTML=rows.map((r,i)=>{const rating=Math.max(0,Math.min(5,Number(r.rating)||0));const stars=rating?`<span class="stars" aria-label="별점 ${rating}점">${'★'.repeat(rating)}${'☆'.repeat(5-rating)}</span>`:'';return `<article class="review-card"><div>${stars}${r.verified?'<span class="verified-review">구매·상담 확인 완료</span>':''}<p>“${esc(r.text)}”</p><em>${esc(r.author||'이룸 고객')}</em></div><img src="${A+fallback[i%fallback.length]}" alt="" loading="lazy" decoding="async"></article>`}).join('')}
async function loadPublicConfig(){try{const r=await fetch('/api/site/footer-config',{cache:'no-store'});if(!r.ok)return;const d=await r.json();publicSite={...publicSite,...(d.site||{})};publicPolicies={...publicPolicies,...(d.policies||{})};const set=(sel,text,hideEmpty=false)=>{const el=$(sel);if(!el)return;if(hideEmpty&&!text){el.hidden=true;return}el.hidden=false;el.textContent=text};set('[data-footer-site]',publicSite.siteName||'이룸 fresh fruits');set('[data-footer-representative]',publicSite.representative?`대표자 ${publicSite.representative}`:'',true);set('[data-footer-business]',`사업자등록번호 ${publicSite.businessNo||'775-97-00292'}`);set('[data-footer-mailorder]',`통신판매업 신고 ${publicSite.mailOrderNo||'제 2025-서울 송파 -1052호'}`);set('[data-footer-address]',publicSite.address||'서울특별시 송파구 송이로 15길 33');set('[data-footer-phone]',publicSite.phone?`고객센터 ${publicSite.phone}`:'',true);set('[data-footer-email]',publicSite.email?`이메일 ${publicSite.email}`:'',true);set('[data-footer-hosting]',`호스팅서비스 제공자 ${publicSite.hostingProvider||'Render'}`);set('[data-trust-shipping]',`${won(publicSite.freeShippingFrom||50000)} 이상 무료배송`);renderPublicReviews(d.reviews||[]);}catch(_){}}
